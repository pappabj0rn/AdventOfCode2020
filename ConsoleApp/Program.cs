...
namespace ConsoleApp
{
    class Program
    {
        ...
        private static Bitmap _bitmap = new Bitmap(98,93);
        ...
        static void UpdateBitmap(char[][] simField)
        {
            for (var y = 0; y < simField.Length; y++)
            {
                for (var x = 0; x < simField[y].Length; x++)
                {
                    var color = simField[y][x] switch
                    {
                        'L' => Color.White,
                        '#' => Color.Red,
                        _ => Color.Black
