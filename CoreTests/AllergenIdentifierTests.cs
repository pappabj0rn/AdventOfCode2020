...
namespace CoreTests
{
    public class AllergenIdentifierTests
    {
        [Fact]
        public void Set_should_split_input_string_into_ingredients_and_allergens()
        {
            var set = new Set("mxmxvkd kfcds sqjhc nhms (contains dairy, fish)");

            Assert.Equal(4, set.Ingredients.Count);
            Assert.Contains("mxmxvkd", set.Ingredients);
            Assert.Contains("kfcds", set.Ingredients);
            Assert.Contains("sqjhc", set.Ingredients);
            Assert.Contains("nhms", set.Ingredients);
            
            Assert.Equal(2, set.Allergens.Count);
            Assert.Contains("dairy", set.Allergens);
            Assert.Contains("fish", set.Allergens);
        }
